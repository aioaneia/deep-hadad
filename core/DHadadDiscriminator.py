...
class DHadadDiscriminator(Module):
    ...
    def to_self_attention_device(self):
        """Ensure that Self-Attention layers are on the same device as the discriminator."""
        device = next(self.parameters()).device
        
        self.self_attention_96.to(device)
        self.self_attention_512.to(device)
