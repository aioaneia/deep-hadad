...
####################################################################################################
project_path = '../'
inscriptions_dataset_path = project_path + config['DEFAULT']['INSCRIPTIONS_DATASET_PATH']
...
if __name__ == "__main__":
    ...
    inscriptions_images = load_inscriptions_images_from_directory(inscriptions_dataset_path, dir_regex=r'test')
    ...
    generate_depth_estimations(inscriptions_images, zoe, displacement_maps_path,
                               invert=True)  # False for Zoe_NK karatepe
...
def test_zoe():
    ...
    for i, img in enumerate(images_only):
        # Predict depth
        depth_pil = predict_depth(zoe, img, invert=True)
